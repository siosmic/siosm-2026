window.addEventListener("load", function () {
  ...
  // Set dynamic CSS variable for header height
  ...
  // Desktop Megamenu controller
  const desktopMegamenuTriggers = document.querySelectorAll(
    "header .menu-item-megamenu",
  );
  const desktopMegamenuPanels = document.querySelectorAll(
    ".megamenu-panel:not(#offcanvas-menu .megamenu-panel)",
  );

  if (desktopMegamenuTriggers.length && desktopMegamenuPanels.length) {
    let activeTimeout = null;

    const closeAll = () => {
      desktopMegamenuPanels.forEach((p) => p.classList.remove("is-open"));
      desktopMegamenuTriggers.forEach((t) => t.classList.remove("is-active"));
    };

    const openPanel = (targetId, trigger) => {
      clearTimeout(activeTimeout);
      const panel = document.getElementById(targetId);
      if (!panel) return;

      desktopMegamenuPanels.forEach((p) => {
        if (p !== panel) p.classList.remove("is-open");
      });
      desktopMegamenuTriggers.forEach((t) => {
    ...
    desktopMegamenuTriggers.forEach((trigger) => {
      const link = trigger.querySelector("a");
      const targetId =
        trigger.getAttribute("data-megamenu") ||
        (link ? link.getAttribute("data-megamenu-target") : null);
    ...
    desktopMegamenuPanels.forEach((panel) => {
